...
#Создание свой маски
custom_threshold_mask = tpf.create_threshold_mask(threshold=10)

#Инверсируем (~) маску порога, чтобы найти фон
bias = ~tpf.create_threshold_mask(threshold=1)
...
lc_pipe.bin(time_bin_size=15).plot(ax=ax_bin, label='pipeline', linewidth=0.5, color='red')
